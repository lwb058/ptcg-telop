...
-   [Node.js](https://nodejs.org/) (必須 v22.12.0 或以上，已在 v22.19.0 LTS 版本測試通過)
...
最簡單的啟動和執行方式，無需設定任何環境。

1. 安裝並啟動 [Docker Desktop](https://www.docker.com/products/docker-desktop/)。
2. 從 [Releases](https://github.com/lwb058/ptcg-telop/releases) 頁面下載 `Docker_Deploy.zip` 並解壓縮到任意空資料夾。
3. 雙擊執行資料夾中的 `start_docker.bat` (Windows) 或 `start_docker.command` (Mac)。
4. 啟動後，在瀏覽器中存取 `http://localhost:9090`。

---
### 🔰 直接安裝 (不使用 Docker)

直接在電腦上安裝和設定 Node.js 和 Python 環境的方法。
...
4.  在解壓縮後的資料夾中，雙擊執行 `install.bat` (Windows) 或 `install.command` (Mac)。腳本將會自動安裝所有必要的元件。
    *※Mac專用：首次安裝時可能會自動執行 Xcode 授權協議的同意流程，這時系統會要求您輸入 Mac 的開機密碼。*
5.  安裝完成後，雙擊 `start.bat` (Windows) 或 `start_MacOS.command` (Mac) 即可啟動系統。
...
1.  **建立宿主目錄並安裝NodeCG**:
    在任意位置建立宿主目錄，將NodeCG作為npm相依項目安裝。
    ```bash
...
4.  **配置素材檔案（圖片、影片）**:
    ...
    3.  解壓縮下載的檔案，並將其中所有內容放置到宿主目錄的 `assets/ptcg-telop/` 下。（如果 `assets/ptcg-telop` 目錄不存在，請手動建立）
...
├── node_modules/
│   └── nodecg/                    (NodeCG核心，透過 npm install 安裝)
├── assets/
│   └── ptcg-telop/
│       ├── element/               (UI元素資料夾)
│       ├── icons/                 (能量、特殊狀態等圖示資料夾)
...
1.  **啟動系統**:
    -   **簡易安裝包用戶**: 雙擊 `start.bat` (Windows) 或 `start_MacOS.command` (Mac) 啟動。
    -   **手動安裝用戶**: 在宿主目錄執行以下指令。
    ```bash
...
-   **執行目錄**: 啟動指令必須在NodeCG的**宿主目錄**（包含 `package.json` 的目錄）下執行。
