...
-   [Node.js](https://nodejs.org/) (v22.12.0以上必須、v22.19.0 LTSでテスト済み)
...
環境構築不要で、最も簡単に起動・運用できる方法です。

1. [Docker Desktop](https://www.docker.com/products/docker-desktop/) をインストールし、起動しておきます。
2. [Releases](https://github.com/lwb058/ptcg-telop/releases) ページから `Docker_Deploy.zip` をダウンロードし、任意の空フォルダに解凍します。
3. フォルダ内の `start_docker.bat` (Windows) または `start_docker.command` (Mac) をダブルクリックして実行します。
4. 起動後、ブラウザで `http://localhost:9090` にアクセスします。

---
### 🔰 直導入 (Dockerを使用しない場合)

PCに直接Node.jsやPython環境を構築する方法です。
...
> **【重要】**
> v1.5.0より前のバージョンから更新する場合、データロジックの根本的な変更により、互換性の問題や予期せぬバグが発生する可能性があります。
> これを防ぐため、更新後は必ず**SETTINGSパネル**にある「**データベースをクリア**」ボタンを押して、既存のデータベースをすべて削除してください。

既に古いバージョンをお使いの方は、「パッチファイル」を使って簡単にアップデートできます。

1.  本リポジトリの [**Releases**](https://github.com/lwb058/ptcg-telop/releases) ページにアクセスします。
2.  最新リリースの`Assets`欄から、`patch_vX.X.X.zip` をダウンロードし、解凍します。
3.  中にある `assets` と `bundles` の2つのフォルダをコピーします。
4.  現在お使いの `nodecg` フォルダの中に直接貼り付け、既存のファイルをすべて上書きしてください。

---
### マニュアルインストール

開発者や、手動で環境を構築したい方向けの手順です。

1.  **ホストディレクトリの作成とNodeCGのインストール**:
    任意の場所にホストディレクトリを作成し、NodeCGをnpm依存としてインストールします。
    ```bash
...
2.  **PTCG-Telopバンドルのクローン**:
    ホストディレクトリ内に`bundles`フォルダを作成し、このリポジトリをクローンします。
    ```bash
...
4.  **アセット（画像・動画素材）の配置**:
    ...
    3.  ダウンロードしたファイルを解凍し、中のファイルをすべてホストディレクトリの `assets/ptcg-telop/` に配置します。（もし `assets/ptcg-telop` ディレクトリが存在しない場合は、作成してください）
...
```
nodecg/                            (ホストランタイムディレクトリ)
├── package.json                   (ホスト用 package.json)
├── node_modules/
│   └── nodecg/                    (NodeCGコア、npm installで導入)
├── assets/
│   └── ptcg-telop/
│       ├── element/               (UI要素のフォルダ)
...
-   **実行ディレクトリ**: 起動コマンドは、必ずNodeCGの**ホストディレクトリ**（`package.json`があるディレクトリ）で実行してください。
